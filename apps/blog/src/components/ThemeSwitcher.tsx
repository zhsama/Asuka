...
export default function ThemeSwitcher() {
	const [currentTheme, setCurrentTheme] = useState<EvaTheme>("eva-01");

	useEffect(() => {
		// 初始化主题
		const savedTheme =
			(localStorage.getItem("eva-theme") as EvaTheme) || "eva-01";
		...
		<DropdownMenu>
			...
			<DropdownMenuContent align="end" className="w-56">
				...
				{themeOptions.map((option) => (
					...
					>
						<div
							className={`${option.colorClass} w-4 h-4 rounded-full border border-white/30 shadow-sm`}
						/>
